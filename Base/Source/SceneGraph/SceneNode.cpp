...
bool CSceneNode::SetEntity(EntityBase* theEntity)
{
	if (theEntity)
	{
		this->ApplyTranslate(theEntity->GetPosition().x, theEntity->GetPosition().y, theEntity->GetPosition().z);
...
void CSceneNode::Update(void)
{
	...
	*/
	...
	if (theEntity)
	{
		theEntity->SetPosition(Vector3(transformation_Matrix.a[12], transformation_Matrix.a[13], transformation_Matrix.a[14]));
	}
}
...
void CSceneNode::UpdateTransformation(MS& modelStack)
{
	modelStack.PushMatrix();

	if (this->GetEntity())
	{
		if (!this->GetEntity()->IsDone())
		{
			modelStack.MultMatrix(this->GetTransform());
		}	
	}
	...
	transformation_Matrix = modelStack.Top();

	if (!theChildren.empty())
	{
		for (vector<CSceneNode*>::iterator it = theChildren.begin(); it != theChildren.end(); ++it)
		{
			(*it)->UpdateTransformation(modelStack);
		}
	}
