...
CSpatialPartition::CSpatialPartition(void)
	...
	, showPartition(false)
{
	overrideDetails.SetDetailLevel(CLevelOfDetails::NO_DETAILS);
}
	...
	for (int i = 0; i<xNumOfGrid; i++)
	{
		for (int j = 0; j < zNumOfGrid; j++)
		{
							...
							i, j) == true)
			{
				...
				if (overrideDetails.GetDetailLevel() == CLevelOfDetails::NO_DETAILS)
				{
				...
				{
					theGrid[i*zNumOfGrid + j].SetDetailLevel((CLevelOfDetails::DETAIL_LEVEL)overrideDetails.GetDetailLevel());
				}
			}
			else
...
void CSpatialPartition::Render(Vector3* theCameraPosition)
{
	...
	for (int i = 0; i<xNumOfGrid; i++)
	{
		for (int j = 0; j<zNumOfGrid; j++)
		{
			if (theGrid[i * zNumOfGrid + j].ListSize() > 0)
			{
				modelStack.PushMatrix();
				modelStack.Translate(xGridSize*i - (xSize >> 1) + theGrid[i*zNumOfGrid + j].size.x / 2, 0.0f, zGridSize*j - (zSize >> 1) + theGrid[i*zNumOfGrid + j].size.z / 2);
				modelStack.PushMatrix();
...
vector<EntityBase*> CSpatialPartition::GetObjects(Vector3 position, const float radius)
{
	...
	if (xIndex > this->GetxNumOfGrid() - 1 || zIndex > this->GetzNumOfGrid() - 1 || xIndex < 0 || zIndex < 0)
	{
		vector<EntityBase*> listOfObjects;
		return listOfObjects;
	}

	vector<EntityBase*> listOfObjects = theGrid[xIndex*zNumOfGrid + zIndex].GetListOfObject();

	for (vector<EntityBase*>::iterator it = listOfObjects.begin(); it != listOfObjects.end();)
	{
		EntityBase* object = *it;

		if ((object->GetPosition() - position).LengthSquared() > radius * radius)
		{
...
{
	float xDistance = (xGridSize*xIndex + (xGridSize >> 1) - (xSize >> 1)) + theGrid[zIndex * xNumOfGrid + xIndex].size.x / 2 - theCameraPosition->x;
	float zDistance = (zGridSize*zIndex + (zGridSize >> 1) - (zSize >> 1)) + theGrid[zIndex * xNumOfGrid + xIndex].size.z / 2 - theCameraPosition->z;
