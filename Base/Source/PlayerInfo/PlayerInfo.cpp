...
void CPlayerInfo::Init(void)
{
	...
	CLuaInterface::GetInstance()->GetGlobal("CPlayerInfoStartPos");
	defaultPosition.Set(CLuaInterface::GetInstance()->GetField("x"), CLuaInterface::GetInstance()->GetField("y"), CLuaInterface::GetInstance()->GetField("z"));
	...
	m_dSpeed = CLuaInterface::GetInstance()->getFloatValue("walkspeed");
...
void CPlayerInfo::SetToJumpUpwards(bool isOnJumpUpwards)
{
	if (isOnJumpUpwards == true)
	{
		m_bJumpUpwards = true;
		m_bFallDownwards = false;
		m_dJumpSpeed = m_dInitialJumpSpeed;
	}
}
...
void CPlayerInfo::UpdateJumpUpwards(double dt)
{
	...
	// s = u * t + 0.5 * a * t ^ 2
	primaryWeapon->jumpOffset -= m_dJumpSpeed * dt;
...
void CPlayerInfo::UpdateFreeFall(double dt)
{
	...
	// Use SUVAT equation to update the change in position and target
	// s = u * t + 0.5 * a * t ^ 2
	primaryWeapon->jumpOffset -= m_dFallSpeed * dt;
...
void CPlayerInfo::Update(double dt)
{
	...
	double camera_speed = 200;

	double camera_yaw = mouse_diff_x * 0.0174555555555556;		// 3.142 / 180.0
	double camera_pitch = mouse_diff_y * 0.0174555555555556;	// 3.142 / 180.0

	float pitch_limit = 85.9f;
		...
		KeyboardController::GetInstance()->IsKeyDown('D'))
	{
		...
		if (KeyboardController::GetInstance()->IsKeyDown('W'))
		{
			Vector3 movement = viewVector.Normalized();
			movement.y = 0;
			movement = movement * (float)m_dSpeed * (float)dt;
			position += movement;
			primaryWeapon->rotateOffset.x += m_dSpeed * dt;
		}
		...
		if (KeyboardController::GetInstance()->IsKeyDown('A'))
		{
			...
			rightUV.y = 0;
			rightUV.Normalize();
			rightUV = rightUV * (float)m_dSpeed * (float)dt;
			position -= rightUV;
			primaryWeapon->rotateOffset.y -= m_dSpeed * dt;
		}
	...
	//Update the camera direction based on mouse move
	{
		...
		{
			double yaw = (-camera_speed * camera_yaw * dt);
			Mtx44 rotation;
			rotation.SetToRotation(yaw, 0, 1, 0);
			viewUV = rotation * viewUV;
			target = position + viewUV;
			...
			primaryWeapon->rotateOffset.y -= yaw;

		}
		{
			double pitch = (-camera_speed * camera_pitch * dt);
			rightUV = viewUV.Cross(up);
			rightUV.y = 0;
			rightUV.Normalize();
			up = rightUV.Cross(viewUV).Normalized();
			Mtx44 rotation;
			if (m_fPitch + pitch < pitch_limit && m_fPitch + pitch > -pitch_limit)
			{
				m_fPitch += pitch;
				rotation.SetToRotation(pitch, rightUV.x, rightUV.y, rightUV.z);
				viewUV = rotation * viewUV;
				...
				primaryWeapon->rotateOffset.x -= pitch;
			}
		}
	}
		...
		position.y == m_pTerrain->GetTerrainHeight(position))
	{
		...
		Music::GetInstance()->playSound("Sounds//jump.wav");
	}
...
void CPlayerInfo::SetPrimaryWeapon(CWeaponInfo* weapon)
{
...
void CPlayerInfo::SetSecondaryWeapon(CWeaponInfo* weapon)
{
