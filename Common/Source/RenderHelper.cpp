...
void RenderHelper::RenderText(Mesh* _mesh, const std::string& _text, Color _color)
{
	...
	for (unsigned i = 0; i < _text.length(); ++i)
	{
		...
		characterSpacing.SetToTranslation((float)(1 + (int)i) * 0.5f, 0.0f, 0.0f); // 1.0f is the spacing of each character, you may change this value
...
void RenderHelper::RenderTextOnScreen(Mesh* _mesh, const std::string& _text, Vector3 translate, float scale, Color _color)
{
	...
	for (unsigned i = 0; i < _text.length(); ++i)
	{
		Mtx44 characterSpacing, MVP;
		//characterSpacing.SetToTranslation((i+0.5f) * 1.0f, 0, 0); // 1.0f is the spacing of each character, you may change this value
		characterSpacing.SetToTranslation((float)(0.26f * (int)(i)), 0.0f, 0.0f); // 1.0f is the spacing of each character, you may change this value
		MVP = GraphicsManager::GetInstance()->GetProjectionMatrix() * GraphicsManager::GetInstance()->GetViewMatrix() * GraphicsManager::GetInstance()->GetModelStack().Top() * characterSpacing;
		currProg->UpdateMatrix44("MVP", &MVP.a[0]);
...
void RenderHelper::Render2DMesh(Mesh* mesh, Vector3 translate, Vector3 scale, Vector3 rotate)
{
	if (mesh)
	{
		...
		//load material
		currProg->UpdateVector3("material.kAmbient", &mesh->material.kAmbient.r);
		currProg->UpdateVector3("material.kDiffuse", &mesh->material.kDiffuse.r);
		currProg->UpdateVector3("material.kSpecular", &mesh->material.kSpecular.r);
		currProg->UpdateFloat("material.kShininess", mesh->material.kShininess);

		// Update textures first if available
		if (mesh->textureID > 0)
		{
			currProg->UpdateInt("colorTextureEnabled", 1);
			GraphicsManager::GetInstance()->UpdateTexture(0, mesh->textureID);
			currProg->UpdateInt("colorTexture", 0);
		}
		else
		{
